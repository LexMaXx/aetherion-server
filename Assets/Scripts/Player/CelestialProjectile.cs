...
public class CelestialProjectile : MonoBehaviour
{
    ...
    public void Initialize(Transform targetTransform, float projectileDamage, Vector3 initialDirection, GameObject projectileOwner = null, List<EffectConfig> skillEffects = null, bool isVisualOnly = false, bool isCrit = false)
    {
        ...
        if (isVisualOnly)
        {
            // ИСПРАВЛЕНИЕ: Для сетевых визуальных снарядов ОСТАВЛЯЕМ автонаведение включенным,
            // чтобы они корректно долетали до цели на клиентах других игроков (магнитный эффект).
            // НЕ отключаем коллайдер - иначе OnTriggerEnter не сработает и снаряд не взорвется!

            Debug.Log($"[CelestialProjectile] 👁️ Визуальный снаряд создан (без урона, С автонаведением и коллизией)");
        }
    ...
    {
        hitSound = sound;
        Debug.Log($"[CelestialProjectile] 🔊 Hit sound установлен: {sound?.name ?? "None"}");
    }
    ...
    private void HitTarget()
    {
        ...
        if (target != null)
        {
            ...
            Debug.Log($"[CelestialProjectile] 🔍 Target: {target.name}, TargetableEntity: {(targetEntity != null ? "✅ Найден" : "❌ NULL")}, IsAlive: {(targetEntity != null ? targetEntity.IsEntityAlive().ToString() : "N/A")}");
            ...
            if (targetEntity != null && targetEntity.IsEntityAlive())
            {
                if (!visualOnlyMode && damage > 0f)
                {
                    ...
                    {
                        Debug.Log($"[CelestialProjectile] ⛔ Цель {targetEntity.GetEntityName()} является союзником. Урон отменён.");
                        DestroySelf();
                        return;
                    }
                ...
                Debug.Log($"[CelestialProjectile] ⚔️ Урон {damage:F1} нанесён {targetEntity.GetEntityName()} (Faction: {targetEntity.GetFaction()})");
            ...
            else
            {
                ...
                if (enemy != null && enemy.IsAlive() && !visualOnlyMode)
                {
                    ...
                    Debug.Log($"[CelestialProjectile] ✅ Урон нанесен NPC (fallback): {damage}");
                ...
                else if (dummy != null && dummy.IsAlive() && !visualOnlyMode)
                {
                    ...
                    Debug.Log($"[CelestialProjectile] ✅ Урон нанесен DummyEnemy (fallback): {damage}");
            ...
            if (!visualOnlyMode && lifeStealPercent > 0 && owner != null)
            {

...
public class TargetIndicator : MonoBehaviour
{
    ...
    /// <summary>
    /// Получить высоту врага (от центра до верхней точки)
    /// </summary>
    private float GetEnemyHeight(Enemy enemy)
    {
        if (enemy == null) return 2.5f;
        ...
        if (renderers.Length > 0)
        {
            ...
            // Возвращаем высоту относительно центра врага
            float height = maxY - enemy.transform.position.y;
            return height;
        }
        ...
        if (collider != null)
        {
            float height = collider.bounds.max.y - enemy.transform.position.y;
            return height;
        }

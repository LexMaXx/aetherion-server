...
public class FogOfWar : MonoBehaviour
{
    ...
    [Tooltip("КРИТИЧНО: Отключить FogOfWar для NetworkPlayer в PvP арене (все игроки всегда видны)")]
    [SerializeField] private bool disableFogForNetworkPlayers = false; // Для BattleScene должно быть TRUE!
    ...
    private void FindAllEnemies()
    {
        ...
        // Логируем каждого врага для отладки
        foreach (Enemy enemy in allEnemies)
        {
            Debug.Log($"[FogOfWar]   - {enemy.GetEnemyName()} на {enemy.gameObject.name}");
        }
    }
    ...
    private void SetEnemyVisibility(Enemy enemy, bool visible)
    {
        ...
        {
            // КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: В PvP арене NetworkPlayer ВСЕГДА ВИДНЫ!
            if (disableFogForNetworkPlayers)
            {
                // PvP Arena Mode: Все игроки всегда видны (как в CS:GO/Valorant)
                Debug.Log($"[FogOfWar] 🏟️ PvP Arena Mode: {networkPlayer.username} ВСЕГДА ВИДИМ (disableFogForNetworkPlayers=true)");

                // Принудительно включаем все рендеры
                ...
                foreach (Renderer rend in renderers)
                {
                    ...
                    {
                        rend.enabled = true;
                        Debug.Log($"[FogOfWar]   ✅ Включен Renderer: {rend.name}");
                    }
                }
                return; // ВАЖНО: Выходим без применения FogOfWar!
            }

            // Это сетевой игрок - применяем Fog of War!
            Debug.Log($"[FogOfWar] NetworkPlayer {networkPlayer.username}: visible={visible}");

            // КРИТИЧЕСКИ ВАЖНО: Проверяем трансформацию перед изменением рендереров!
            ...
            if (isTransformed)
            {
                Debug.Log($"[FogOfWar] ⚠️ {networkPlayer.username} трансформирован! Управляем только моделью трансформации.");

                // Если трансформирован - управляем только рендерерами ВНУТРИ TransformedModel_Visual
                ...
                if (transformedModel != null)
                {
                    ...
                    Debug.Log($"[FogOfWar] Трансформированная модель {networkPlayer.username}: {transformedRenderers.Length} рендереров установлены в visible={visible}");
                }

                // ВАЖНО: Оригинальная модель паладина ОСТАЕТСЯ СКРЫТОЙ!
                // Не трогаем рендереры паладина и его оружия!
            }
            else
            {
                // Обычный режим - управляем всеми рендерерами
                Renderer[] renderers = networkPlayer.GetComponentsInChildren<Renderer>();
                foreach (Renderer rend in renderers)
                {
                    rend.enabled = visible;
                }
                Debug.Log($"[FogOfWar] {networkPlayer.username}: {renderers.Length} рендереров установлены в visible={visible}");
            }
        ...
        // Обычный NPC враг (DummyEnemy, мобы и т.д.)
        Debug.Log($"[FogOfWar] 🎯 {enemy.GetEnemyName()}: visible={visible}");

        Renderer[] npcRenderers = enemy.GetComponentsInChildren<Renderer>();
        Debug.Log($"[FogOfWar]   Renderer'ов найдено: {npcRenderers.Length}");
        ...
        foreach (Renderer rend in npcRenderers)
        {
            if (rend.enabled != visible)
            {
                rend.enabled = visible;
                changedCount++;
                Debug.Log($"[FogOfWar]   {(visible ? "✅ Показан" : "❌ Скрыт")}: {rend.name}");
            }
        }

        if (changedCount == 0 && npcRenderers.Length > 0)
        {
            Debug.Log($"[FogOfWar]   ⚠️ Все Renderer'ы уже были в состоянии visible={visible}");
        }
    ...
    /// КРИТИЧЕСКОЕ: Сразу проверяем видимость для NetworkPlayers!
    /// </summary>
    public void RegisterEnemy(Enemy enemy)
    {
        if (!allEnemies.Contains(enemy))
        {
            ...
            // ВАЖНО: Сразу инициализируем состояние видимости
            if (!enemyVisibilityStates.ContainsKey(enemy))
            {
                enemyVisibilityStates[enemy] = new EnemyVisibilityState
                {
            ...
            // КРИТИЧЕСКОЕ: Сразу проверяем видимость!
            // Это гарантирует что NetworkPlayer появится сразу, а не через 0.2 сек
            if (player != null && enemy != null)
            {
                ...
                if (shouldBeVisible)
                {
                    ...
                    SetEnemyVisibility(enemy, true);
                    Debug.Log($"[FogOfWar] ✅ Враг {enemy.GetEnemyName()} СРАЗУ ВИДЕН (расстояние: {distance:F1}м)");
                }
                else
                {
                    SetEnemyVisibility(enemy, false);
                    Debug.Log($"[FogOfWar] ⚠️ Враг {enemy.GetEnemyName()} скрыт (расстояние: {distance:F1}м > {visibilityRadius}м)");
                }
            }
        }
    }
